...
이 5주차 구현은 Claude Code(Opus 4.8)와 페어 프로그래밍으로 진행했습니다.

- **설계 후 리뷰 **: 상태 표 작성 후 리뷰 진행 상태 소유권 분류, staleTime/gcTime 정책, 목록 카드에 품절 제외·할인 포함, selector 구독 경계 등을 대화로 근거를 세워 결정했습니다.
- **코드 생성후 리뷰 **: 각 파일을 검토하고 lint·타입 검사·빌드·수동 동작으로 검증했습니다.
- 모든 커밋에 `Co-Authored-By: Claude` 트레일러로 표기했습니다.
